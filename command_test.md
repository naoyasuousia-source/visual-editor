...
----------------------------------------
## 1. 未解決要件
**以下はすでに実装されているはずだが、機能していないので機能するようにする。**
・自動挿入成功後、全体承認or全体破棄のバーをメニューバーの下に表示。
・自動挿入成功後、変更箇所ごとにホバーで承認or破棄を選択可能。
・全体承認、破棄が選択されたら、部分選択されていない段落が残っていても、自動保存後、自動編集フローは終了とする
