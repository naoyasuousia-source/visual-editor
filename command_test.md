...
### 2025-12-30 HTMLコメントパーサー修正とAIガイド最終更新（完了）

#### 修正ファイル9: `src/v2/utils/htmlCommentParser.ts`
**問題:**
- test-commands-comprehensive.txtのテストセット1（HTMLコメント形式）をコマンドエリアに入れても認識されない
- 有効コマンドとして処理されず、保護上書きが発動する
- extractCommandFromCommentsの正規表現が旧コマンド形式`^[A-Z_]+\[`にマッチするパターンだった
- 新コマンド形式`REPLACE_PARAGRAPH(`は`(`を使うため、このチェックに引っかかって除外されていた

**方針:**
- 旧コマンド形式のチェック`/^[A-Z_]+\[/`を削除
- 新コマンド形式専用のパターンマッチングに変更
- 6種類の新コマンドを明示的に指定

**変更内容:**
- 不要な除外条件を削除（`content.startsWith('テスト')`、コロンチェック）
- 正規表現を新コマンド専用に変更:
  ```typescript
  ...
  ```
- HTMLコメント`<!-- COMMAND(...) -->`から正しくコマンド文字列を抽出できるように修正

#### 修正ファイル10: `src/v2/utils/aiMetadata.ts`
**問題:**
- AIエージェントがHTMLコメント形式を理解せず、素のコマンド文字列を記述してしまう
- 既存のガイドではHTMLコメント形式の重要性が十分に強調されていない
...
**変更内容:**
- **新セクション追加**:
  - `⚠️ COMMAND FORMAT RULES (MUST READ!)` - HTMLコメント形式の説明
  - `COMPLETE WORKING EXAMPLE` - 完全な動作例
  - `CRITICAL RULES (READ CAREFULLY!)` - 重要ルールの強調
  - `⚠️ FINAL REMINDER` - 最終リマインダー
- **すべての例を`<!-- -->`形式に変更**
- **正しい形式と間違った形式を対比表示**:
  ```
  ✅ CORRECT FORMAT:
...
**効果:**
- HTMLコメントパーサーが新コマンドを正しく認識
- AIエージェントが確実に正しい形式でコマンドを記述
- テストコマンドが期待通りに機能
