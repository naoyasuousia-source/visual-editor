...
----------------------------------------
## 1. 未解決要件
・外部上書き検知～全体承認or全体破棄確定まではエディタをロックし、ユーザーは一切編集できないようにする。→**再修正：App.tsxにて「自動編集中」または「未承認ハイライトあり」の間、Tiptapの更新イベント等をフックして強制的にロック（setEditable(false)）を維持するロジックを導入。** (※解決するまでOKにしないこと)
