...
----------------------------------------
## 1. 未解決要件
・外部上書き検知～全体承認or全体破棄確定まではエディタをロックし、ユーザーは一切編集できないようにする。→**ロックできていない**
・自動挿入成功後、全体承認or全体破棄のバーをメニューバーの下に表示。→**OK**
・自動挿入成功後、変更箇所ごとにホバーで承認or破棄を選択可能。→**できていない**
・全体承認、破棄が選択されたら、部分選択されていない段落が残っていても、自動保存後、自動編集フローは終了とする。→**個別メニュー未実装のため未確認**
...
### 承認バーおよび個別承認ポップアップの表示不具合 & エディタのロック
- **問題**: Zustandストアへの購読（Selector）が不十分で、ハイライト登録後にUIが自動更新されていなかった。また、ホバーイベントがバブリングの途中で遮断されたり、マウス移動中にタイマーがクリアされてポップアップが表示されないケースがあった。
- **解決方法**:
  - `useCommandApprovalController.ts` において、`useCommandHighlightStore` から `highlights` と `pendingCount` をセレクタで直接購読。
  - ホバーイベントをキャプチャーフェーズでリスンし、かつ `relatedTarget` をチェックすることで段落内でのマウス移動によるタイマーリセットを防止。
  - `useAutoEdit.ts` および `useCommandApprovalController.ts` において、保留中のハイライトがある間は `editor.setEditable(false)` を維持するように改善。
  - `EditorLockOverlay.tsx` の表示条件を整理。
