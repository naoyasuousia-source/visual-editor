...
・自動挿入成功後、変更箇所ごとにホバーで承認or破棄を選択可能。→**OK**
・全体承認、破棄が選択されたら、部分選択されていない段落が残っていても、自動保存後、自動編集フローは終了とする。→**OK**
...
### 承認バーおよび個別承認ポップアップの表示不具合
- **問題**: Zustandストアへの購読（Selector）が不十分で、ハイライト登録後にUIが自動更新されていなかった。また、ホバーイベントがバブリングの途中で遮断され、ポップアップが表示されないケースがあった。
- **解決方法**:
  - `useCommandApprovalController.ts` および `useCommandHighlight.ts` において、`useCommandHighlightStore(state => state.highlights)` を使用して明示的に状態変更を監視するように修正。
  - ホバーイベント（mouseover/mouseout）をキャプチャーフェーズ（`useCapture: true`）でリスンするように変更し、イベント遮断を回避。
  - `App.tsx` のレイアウトを調整し、承認バーをツールバー直下の使いやすい位置に配置。
  - 各アクション完了後に `saveFile` を自動実行するように統合。
