...
### v2.1 (2025-12-29 12:39)
- **ハイライト仕様変更**
  - 変更箇所のカラー表示を承認/破棄まで維持（フェードアウトしない）
  - 承認/破棄待ち中はエディタをロック（手動編集不可）
- **コマンド取扱説明書の埋め込み**
  - content.cssにAIエージェント向けコマンド仕様をHTMLコメントで埋め込む
...
#### ステップ3: コマンド実行とファイルの正規化保存
- エディタ内でコマンドを順次実行
- **重要**: 実行後のエディタ状態から正規のHTMLを再構築
  - これにより、AIエージェントがコマンドエリア以外を勝手に編集していても、その変更は破棄される（セキュリティ強化）
- 新しいコマンドエリア（空）を注入
- ファイルを上書き保存
...
#### ステップ5: 結果表示
- **成功時**:
  - 変更箇所をカラー表示（黄色背景、**承認/破棄まで維持**）
  - AutoEditBarを表示
  - **エディタをロックしたまま（`setEditable(false)`を維持）**
- **失敗時**:
  - エディタロックを解除
  - エラーメッセージをトースト表示
  - AutoEditBarは表示しない
