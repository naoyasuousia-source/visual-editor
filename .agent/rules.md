...
- **Optimal Resolution:** 解決困難な要件の解決においては、ルート直下に、専用のmdファイルを作成する。ファイルの内容は以下のとおりとし、ほかのモデルも即座に状況を把握できるようにする。1.未解決要件、2.未解決要件に関するコード変更履歴（毎回、分析結果・方針・変更内容を詳細に記述する）3.分析中に気づいた重要ポイント（試してだめだったこと、制約条件etc...）、4.解決済み要件とその解決方法、5.要件に関連する全ファイルのファイル構成、6.要件に関連する技術スタック、7.要件に関する機能の動作原理（依存関係含む）
