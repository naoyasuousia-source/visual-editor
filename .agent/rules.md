...
- **Optimal Resolution:** 解決困難な要件の解決においては、ルート直下に、専用のmdファイルを作成する。ファイルの内容は以下のとおりとし、ほかのモデルも即座に状況を把握できるようにする。1.要件、2.現在の問題点、3.要件に関連する全ファイルのファイル構成、4.要件に関連する技術スタック、5.動作原理、6.コードの変更履歴（毎回、変更の目的と変更内容を詳細に記述する）、7.デバッグ中に気づいた重要ポイント
