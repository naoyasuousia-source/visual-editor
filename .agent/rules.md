...
- **Error Logs:** 解決困難なバグや設計変更が発生した際は `error-logs.md` に原因と対策を記録し、自律的に実装計画を更新せよ。
...
- **view_file_outline 優先**: 長いファイル（200行以上）を初めて確認する際は、`view_file_outline` でファイル構造を把握してから、必要箇所のみ `view_file` で詳細確認せよ。
- **grep_search 活用**: 特定の文字列・パターンを探す場合は、`view_file` で全行読み込みせず、`grep_search` で該当箇所を特定せよ。
