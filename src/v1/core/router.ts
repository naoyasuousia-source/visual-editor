...
export type EditorMode = 'standard' | 'word';
...
export function getMode(): EditorMode {
    return (localStorage.getItem(STORAGE_KEY) as EditorMode) || 'standard';
}

function setModeInternal(mode: EditorMode) {
...
export function initRouter() {
    ...
    window.addEventListener('hashchange', () => {
        ...
        let nextMode: EditorMode | null = null;
        if (newHash === HASH_WORD) nextMode = 'word';
        if (newHash === HASH_STANDARD) nextMode = 'standard';
