...
let openedFileHandle: any = null; // FileSystemFileHandle
...
export function setPagesHTML(html: string): void {
  ...
  if (window.applyFontFamily) {
    ...
    if (style) window.applyFontFamily(style);
  }

  if (window.renumberPages) window.renumberPages();
  if (window.initPages) window.initPages();
}
...
export function handleOpenFile(event: Event): void {
...
export function buildFullHTML(): string {
  ...
  const pagesContainerClone = pagesContainerElement.cloneNode(true) as HTMLElement;
  pagesContainerClone
...
export function saveFullHTML(): void {
