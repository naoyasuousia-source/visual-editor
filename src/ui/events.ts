...
export function bindParagraphMenuListeners(): void {
    ...
    paragraphSubmenuTriggerElements.forEach(trigger => {
        ...
        trigger.addEventListener('click', (event) => {
            ...
            if (window.closeAllParagraphSubmenus) window.closeAllParagraphSubmenus();
            submenu.classList.toggle('is-open', willOpen);
            trigger.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
            if (willOpen) {
                if (window.setParagraphMenuOpen) window.setParagraphMenuOpen(true);
            }
        });
    });
}
    ...
    toolbarElement.addEventListener('click', async (event) => {
        ...
        switch (action) {
            ...
            case 'paragraph-style':
                ...
                break;
            case 'align-left':
                if (window.applyParagraphAlignment) window.applyParagraphAlignment('left');
                break;
            case 'align-center':
                if (window.applyParagraphAlignment) window.applyParagraphAlignment('center');
                break;
            case 'align-right':
                if (window.applyParagraphAlignment) window.applyParagraphAlignment('right');
                break;
            case 'paragraph-spacing':
                if (window.applyParagraphSpacing) window.applyParagraphSpacing(btn.dataset.size ?? null);
                break;
            case 'line-height':
                if (window.applyLineHeight) window.applyLineHeight(btn.dataset.size ?? null);
                break;
            case 'block-element':
