...
<requirement>
<content>外部上書き検知～全体承認or全体破棄確定まではエディタをロックし、ユーザーはメニューバーを含め、一切編集できないようにする。全体承諾or破棄ボタンと個別承認or破棄ボタン以外の操作は不可。また、全体承諾メニューで×ボタンは消す。</content>
<current-situation>編集ロックはできている。メニューバーもロックしたい。また、個別の承認or破棄のホバーメニューがでなくなった。</current-situation>
<remarks></remarks>
<permission-to-move>NG</permission-to-move>
</requirement>
...
- **物理レベル**: CSS (`pointer-events-none`) によりマウス操作、フォーカス、テキスト選択を完全に遮断。
- **論理レベル**: Tiptap (`setEditable(false)`) により、キーボード入力やプログラムによるドキュメント変更を禁止。
- **三重ガード (App.tsx)**:
