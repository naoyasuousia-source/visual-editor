...
・自動編集箇所を色付けハイライトする仕様のはずなのにできていない。自動編集後、ユーザーが承認または破棄するまで、色付けハイライトし続けるようにしたい。→**挿入した文字列をハイライトしたいのに、挿入文字列の直前の一字のみがハイライトされ、しかも、承認後もハイライトが消えない**→**未確認。**
...
→**惜しい。AI ASSISTANT GUIDE & COMMAND API SPECIFICATIONSも先頭付近にまとめて記述してほしい。また、AIが実際にコマンドを入力するコマンドエリアもあらかじめ、htmlにそなえつけるようにしたい**
...
→**できてない。自動編集許可ダイアログが出ない。コンソールではコマンドエリアなしとはでるが、現状は、AIによるコード破壊が可能な状態である。**
