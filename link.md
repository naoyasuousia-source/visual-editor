...
・自動編集時は、失敗ダイアログ出ないが、承認後に失敗ダイアログでてしまう。→変更検知後に、コマンドが有効か無効かを判断し、有効なら修正許可ダイアログ、無効なら、失敗ダイアログでるようにし、承認後も失敗ダイアログは絶対出ないようにする。
・失敗ダイアログでは「キャンセル」の選択肢を無くし、エディタロックかつOKのみで、OKを押すと、エディタの状態で上書き保存されるようにする（AIの不正書き換え防止）
