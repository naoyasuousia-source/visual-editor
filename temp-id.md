...
- 2025-12-31: `useCommandHighlight.ts` を修正。ハイライトが新しいコマンドによって「上書き（上乗せ）」された際、上書きされた古いコマンドIDに関連付けられている全てのDOMノードから属性を物理的に削除するように変更。これにより、SPLIT後に一方のみを操作した場合に他方にハイライトが残る「ゴーストハイライト」問題を解消。
...
- **ゴーストハイライトの自動除去**: 連続編集でハイライトが更新（supersede）される際、古いコマンドIDを持つ全ノードのハイライト属性をDOMから削除するロジックを実装。SPLIT後の個別操作など、複数ノードにまたがる操作の整合性を確保。
