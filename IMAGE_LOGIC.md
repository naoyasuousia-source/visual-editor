...
### 2025-12-28 19:10 - サイズ変更機能の完全復旧とCSS詳細度の適正化
- **分析結果**: 
  - **CSS詳細度問題**: `.page-inner .image-container` が持つ `width: auto` が、単一クラスの `.img-s` 等を上書きしていた。これが「サイズが変わらない」最大の原因。
  - **NodeView更新ロジック不備**: `update` 関数内で `className` 全体を上書きしていたため、タイトル表示用のクラス（`has-title` 等）が消失していた。
  - **属性更新の確実性**: Tiptapの `updateAttributes` は NodeSelection 前提の挙動をするため、`selectable: false` の環境では `setNodeMarkup` を直接使う方が確実。
- **方針**:
  - CSS詳細度を上げ、サイズクラスが確実に `width: auto` を上書きするように修正。
  - `customImage.ts` の `update` 処理を `classList` を使った部分更新に変更し、他クラスへの干渉を排除。
  - バブルメニュー等のロジックを `setNodeMarkup` に統一。
- **結果**: ✅ 正常動作確認済み
...
- **CSS詳細度（Specificity）の罠**: コンテナに対して汎用的なセレクタ（`.page-inner .image-container`）で `width: auto` を設定している場合、後から追加するサイズクラス（`.img-s`）も同等以上の詳細度を持たせないと上書きできない。このため `.page-inner .image-container.img-s` のように連結セレクタを用いて優先順位を担保した。
- **NodeViewのupdateメソッド**: Tiptap/ProseMirrorのNodeView.updateは、属性変更時に既存のDOMを再利用するための重要な場所。ここで `className` 全体を書き換えると、CSS擬似要素表示用のクラスなどが消えてしまうため、`classList` で差分だけを管理するのが鉄則。
...
- **副作用の修正**: コンテナへのクラス移行時に発生した CSS 詳細度不足と `className` リセットによるバグ（サイズ変更不能、タイトル消失）を解消。
