...
## 3. 分析中に気づいた重要ポイント
- **キャレットの吸着特性**: ブラウザのテキストキャレットは、インライン要素の `content` エッジに吸着する。`margin` や `padding` は無視されることが多いが、`letter-spacing` は「文字の送り幅」として認識されるため、キャレット位置を制御するのに有効。
- **インラインブロックの高さ**: `display: inline-block` を持つ画像コンテナは、親要素の `line-height` が `normal` や数値（1.2等）の場合、ベースライン基準で高さが計算され、キャレットが画像の一部（下部など）にしか表示されない。`line-height: 1` にすることで、行の高さがオブジェクトの高さそのものになり、キャレットが垂直方向をフルにカバーする。
- **擬似要素によるタイトル表現**: タイトルをDOMとして持つと、その中にキャレットが入り込んでしまう問題がある。`::after` で表現することで、データとしてのタイトル（属性）と表示上のタイトルを分離し、エディタからは「単一の文字（画像）」として扱わせることに成功した。
...
- **キャレットの間隔制御**: `letter-spacing` を画像段落に適用することで、画像コンテナ（インラインブロック）と直後のテキストキャレットとの間に物理的な余白を確保している。
