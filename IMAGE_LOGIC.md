...
- **現状**: `margin-right: 1.5pt`を画像コンテナに適用 → **動作確認待ち**

### 1.2 画像上辺と段落番号の位置関係
- **要件**: 画像段落では、段落番号のy座標と画像上辺のy座標を一致させる。
- **現状**: `vertical-align: top`を画像コンテナに適用 → **動作確認待ち**

### 1.3 タイトル挿入時の段落高さ調整
- **要件**: タイトル挿入時は、タイトルの高さ分だけ段落高さを増やしたい（タイトル挿入時と未挿入時で画像段落下辺と次段落との距離を一定にしたい）
- **現状**: タイトルサイズ別の`margin-bottom`を適用（本文: 1.8em, サブテキスト: 1.5em）→ **動作確認待ち**

---

## 2. 未解決要件に関するコード変更履歴

### 2025-12-28 18:05 - 未解決要件1.1, 1.2, 1.3への対応（動作確認待ち）
- **分析結果**:
  - 1.1: 前回`letter-spacing`で失敗した可能性を考慮し、`margin-right`方式を採用
  - 1.2: `vertical-align: bottom`から`top`に変更で画像上辺を揃える
  - 1.3: タイトル擬似要素の高さに応じた`margin-bottom`を計算（本文11pt × 1.4 + 4px ≈ 1.8em）
- **方針**:
  - `letter-spacing`は他のテキストにも影響するため、`margin-right`のほうが画像コンテナに限定した調整が可能
  - `vertical-align: top`で画像上辺を行ボックス上辺に揃える
  - タイトルサイズ（本文/サブテキスト）に応じた2段階の`margin-bottom`
- **変更内容**:
  ```css
  /* content.css の変更 */
  .page-inner .image-container {
    vertical-align: top; /* 【1.2】画像上辺を段落番号に合わせる */
    margin-right: 1.5pt; /* 【1.1】キャレット位置を右に1.5pt */
  }
  
  .page-inner .image-container.has-title {
    margin-bottom: 1.8em; /* 【1.3】本文タイトル高さ分 */
  }
  
  .page-inner .image-container.has-title.title-mini {
    margin-bottom: 1.5em; /* 【1.3】サブテキストタイトル高さ分 */
  }
  ```
...
### 3.4 キャレット位置調整に関する分析（2025-12-28）

#### ⚠️ `letter-spacing`でのキャレット位置調整は要注意
- `letter-spacing`は画像段落内のすべての要素に影響する可能性がある
- 意図しない副作用が発生する可能性があるため、画像コンテナに限定した`margin-right`のほうが安全

#### ✅ `margin-right`でキャレット位置を調整
- 画像コンテナに`margin-right: 1.5pt`を設定することで、キャレットを右に1.5pt移動できる
- この方法は画像コンテナのみに影響し、他のテキストには影響しない
- **動作確認待ち**

#### ✅ `vertical-align: top`で画像上辺を揃える
- `vertical-align: bottom`から`top`に変更することで、画像上辺が行ボックスの上辺に揃う
- 段落番号は`top: 1px`で絶対配置されているため、画像上辺もほぼ同じy座標に配置される
- **動作確認待ち**

#### ✅ タイトルサイズに応じた`margin-bottom`
- タイトル擬似要素は`position: absolute`でドキュメントフローから除外されているため、自動的には余白を取らない
- タイトルの実際のサイズ（本文: 11pt × 1.4 ≈ 1.8em、サブテキスト: ≈ 1.5em）に応じた`margin-bottom`を設定
- CSS擬似要素の高さはJSから測定困難なため、固定値での対応が現実的
- **動作確認待ち**
